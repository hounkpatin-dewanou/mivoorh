...
- Ports libres : **8000** (application), MySQL interne au réseau Docker
...
| URL | Rôle |
|-----|------|
| http://localhost:8000 | Application (landing + SPA) |
| http://localhost:8000/api/health | Santé API (`{"status":"ok"}`) |
...
1. Ouvrir http://localhost:8000 — parcourir la landing.
...
5. **Inscription** http://localhost:8000/register :
...
├── docker-compose.yml       # Stack MySQL + API + nginx (port 8000)
└── README.md
```
...
| Problème | Piste |
|----------|--------|
| Port 8000 occupé | Changer `8000:80` dans `docker-compose.yml` |
